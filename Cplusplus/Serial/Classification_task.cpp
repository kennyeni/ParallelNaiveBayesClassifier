...
void Classification_task::initialize()
{
	...
	if (!initialized){
		...
		for (int i = 0; i < MAX_CATEGORIES; i++){
			Model_ReduceHashMap::iterator a;
			string label(categories[i]);
			a = modelDictionary->find(label + "_class");
	...
	for (int i = 0; i < 50; i++){
		...
		if (featureValue == 1){
			for (int i = 0; i < MAX_CATEGORIES; i++){
				...
				if (modelDictionary->end() == (a = modelDictionary->find(find))){ // This feature was unknown always, skip it
